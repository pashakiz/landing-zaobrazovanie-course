...
jQuery(document).ready(function($) { //in jquvery-1.11.1.min.js use this: var $ = jQuery.noConflict();
	...
	// 	var client_name, client_email, client_middlename, client_surname;
	// 	client_name = $.trim($('#client_name').val());
	// 	client_email = $.trim($('#client_email').val());
	// 	client_middlename = $.trim($('#client_middlename').val());
	// 	client_surname = $.trim($('#client_surname').val());
	...
	// 			$('#order_status').html('Введите, пожалуйста, правильный e-mail').addClass("error");
	// 		}
	// 		if (obj.status == "badRequest"){
	// 			$('#order_status').html('Bad request').addClass("error");
	// 		}
	// 		if (obj.status == "allValuesRequired"){
	// 			$('#order_status').html('Все поля обязательны для заполнения').addClass("error");
	// 		}
	// 	});
